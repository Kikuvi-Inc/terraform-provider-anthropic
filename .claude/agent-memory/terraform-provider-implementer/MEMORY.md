...
- [Provider backend/key model](project_api_key_model.md) — Standard resources use pd.Client (ANTHROPIC_API_KEY); admin/org resources use pd.AdminClient (ANTHROPIC_ADMIN_API_KEY); third backend is Claude Platform on AWS (aws block) where Admin is unavailable but pd.Client stays *anthropic.Client
